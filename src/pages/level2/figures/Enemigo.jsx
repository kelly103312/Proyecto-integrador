...
export function Model(props) {
  ...
  const [currentAction, setCurrentAction] = useState('Rest'); // Animación inicial
  ...
  useFrame(() => {
    ...
    // Calcular la distancia al avatar
    const distanceToAvatar = modelPosition.distanceTo(avatarPosition);

    // Ajustar la velocidad de movimiento en función de la distancia al avatar
    const speed = distanceToAvatar < 1 ? 0.01 : 0.05; // Movimiento lento cuando está cerca
    ...
    // Cambiar a la animación "RideIdle" si está cerca del avatar y restar vidas
  ...
  return (
    <group ref={group} {...props} dispose={null}>
      <group name="Scene">
        <group name="Sketchfab_model" rotation={[-Math.PI / 2, 0, 0]} scale={0.10}>
          <group name="root">
