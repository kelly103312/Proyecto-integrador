...
export default function Sensei(props) {
  ...
  const position = vec3();
  const rotation = quat();
  const rigidBodySenseiRef = useRef();
  ...
  useEffect(() => {
    setAvatar({
      ...avatar,
      senseiRef: senseiRef?.current,
    });
  }, [senseiRef.current, setAvatar]);
  ...
  socket.on("updates-values-transform-player", (player) =>{
    ...
    rigidBodySenseiRef.current?.setTranslation(position, true)
    rigidBodySenseiRef.current?.setRotation(rotation, true)
})
  ...
  return (
    ...
    <RigidBody ref={rigidBodySenseiRef} colliders={false} type="fixed" position={[ 0, 0, -59]}>
      <group ref={senseiRef} name="Scene" position-y={-0.9}>
